package sim.app.episim.model.biomechanics.centerbased2Df.oldmodel.wound;
...
public class AdhesiveCenterBased2DModel extends AbstractCenterBased2DModel {
   ...
   private AdhesiveCenterBased2DModelGP globalParameters = null;
   ...
   public AdhesiveCenterBased2DModel(AbstractCell cell){
      ...
      if(cell != null && cell.getMotherCell() != null && cell.getMotherCell().getEpisimBioMechanicalModelObject() != null){
      	...
      	motherCellWidth= ((AdhesiveCenterBased2DModel)(cell.getMotherCell().getEpisimBioMechanicalModelObject())).getKeratinoWidth();
	      double deltaX = 0;
	      if(((AdhesiveCenterBased2DModel)(cell.getMotherCell().getEpisimBioMechanicalModelObject())).dividesToTheLeft){
	      ...
	      if(cell.getMotherCell().getEpisimBioMechanicalModelObject() instanceof AdhesiveCenterBased2DModel){
	      	EpisimModelConnector motherCellConnector =((AdhesiveCenterBased2DModel) cell.getMotherCell().getEpisimBioMechanicalModelObject()).getEpisimModelConnector();
	      ...
	      Double2D oldLoc=cellField.getObjectLocation(cell.getMotherCell());	   
	       if(oldLoc != null){
		      ...
		      cellField.setObjectLocation(cell, newloc);		      
		 	  	SimulationDisplayProperties props = ((AdhesiveCenterBased2DModel)cell.getMotherCell().getEpisimBioMechanicalModelObject()).getCellEllipseObject().getLastSimulationDisplayProps();
   ...
   public HitResult hitsOther(Bag neighbours, Double2D thisloc, boolean finalPosition)
   {
       ...
       for(int i=0;i<neighbours.numObjs;i++)
       {
          ...
          if (other != getCell())
          {
         	 AdhesiveCenterBased2DModel mechModelOther = (AdhesiveCenterBased2DModel) other.getEpisimBioMechanicalModelObject();
   ...
   public void newSimStep(long simstepNumber){
   	...
   	if(ModelController.getInstance().getEpisimBioMechanicalModelGlobalParameters() instanceof AdhesiveCenterBased2DModelGP){
   		globalParameters = (AdhesiveCenterBased2DModelGP) ModelController.getInstance().getEpisimBioMechanicalModelGlobalParameters();
   	}   	
   ...
   public GenericBag<AbstractCell> getDirectNeighbours(){
   	...
   	for(int i=0;neighbours!= null &&i<neighbours.size();i++)
      {
       ...
       {
      	 AdhesiveCenterBased2DModel mechModelOther = (AdhesiveCenterBased2DModel) actNeighbour.getEpisimBioMechanicalModelObject();
	...
	public void calculateClippedCell(long simstepNumber){
    	...
    	if(neighbourhood != null && neighbourhood.size() > 0 && cellEllipseCell.getLastSimulationDisplayProps()!= null){
    		for(AbstractCell neighbouringCell : neighbourhood){
    			if(neighbouringCell.getEpisimBioMechanicalModelObject() instanceof AdhesiveCenterBased2DModel){
    				AdhesiveCenterBased2DModel biomechModelNeighbour = (AdhesiveCenterBased2DModel) neighbouringCell.getEpisimBioMechanicalModelObject();
   ...
   protected void newSimStepGloballyFinished(long simStepNumber, SimState state){
      ...
      if(allCells!= null){
	      ...
	      for (int i=0; i<numberOfCells; i++)
	      {
	          ...
	          if(cellArray[i]!=null){
	         	 AdhesiveCenterBased2DModel mechModel = (AdhesiveCenterBased2DModel)cellArray[i].getEpisimBioMechanicalModelObject();
	      ...
	      for (int k=0; k< MAX_XBINS; k++)
	      {
	          if((xLookUp[k]==null)) continue; // stem cells cannot be outer cells (Assumption)                        
	         	 ...
	         	 AdhesiveCenterBased2DModel mechModel = (AdhesiveCenterBased2DModel)xLookUp[k].getEpisimBioMechanicalModelObject();
