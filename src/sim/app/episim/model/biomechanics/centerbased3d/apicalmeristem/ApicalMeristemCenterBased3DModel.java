...
public class ApicalMeristemCenterBased3DModel extends AbstractCenterBased3DModel{
   ...
   private EpisimApicalMeristemCenterBased3DMC modelConnector;
   
   private ApicalMeristemCenterBased3DModelGP globalParameters = null;
   
   private double migrationDistWholeSimStep = 0;
   
	private static Continuous3DExt cellField;
	
   private static double FIELD_RESOLUTION_IN_MIKRON			  				= 7;  // Equal to cell diameter, I suppose? ---> Not necessarily
   ...
   public void setEpisimModelConnector(EpisimModelConnector modelConnector){
   	...
   	if(modelConnector instanceof EpisimApicalMeristemCenterBased3DMC){
   		this.modelConnector = (EpisimApicalMeristemCenterBased3DMC) modelConnector;
   ...
   public InteractionResult calculateRepulsiveAdhesiveAndChemotacticForces(Bag neighbours, Double3D thisloc, boolean finalSimStep)
   {
       ...
       for(int i=0;i<neighbours.numObjs;i++)
       {
          ...
          ApicalMeristemCenterBased3DModel mechModelOther = (ApicalMeristemCenterBased3DModel) other.getEpisimBioMechanicalModelObject();
            ...
            // Attraction forces 
             ...
             if(this.modelConnector instanceof episimmcc.centerbased3d.apicalmeristem.EpisimApicalMeristemCenterBased3DMC && finalSimStep){
          		...
          		//			dy, thisSemiAxisA, thisSemiAxisB, thisSemiAxisB, otherSemiAxisA, otherSemiAxisB,otherSemiAxisC, d_membrane_this, d_membrane_other, actDist, optDist);
          			...
          			((episimmcc.centerbased3d.apicalmeristem.EpisimApicalMeristemCenterBased3DMC)this.modelConnector).setContactArea(other.getID(), Math.abs(contactAreaCorrect));
	...
	public void calculateSimStep(boolean finalSimStep){
		if(finalSimStep){
			...
			if(modelConnector instanceof episimmcc.centerbased3d.apicalmeristem.EpisimApicalMeristemCenterBased3DMC){				
		  		 ((episimmcc.centerbased3d.apicalmeristem.EpisimApicalMeristemCenterBased3DMC) modelConnector).getContactArea().clear();
		  	}
   ...
   private void updateDirectNeighbours(){
   	...
   	for(int i=0;neighbours != null && i<neighbours.size();i++)
      {
	       ...
	       {
	  			ApicalMeristemCenterBased3DModel mechModelOther = (ApicalMeristemCenterBased3DModel) actNeighbour.getEpisimBioMechanicalModelObject();
...
//	   			System.out.println("Min threshold reached: Interrupted Biomechanical calculation");
	   		...
	   		if(parallelizationOn){
		   		Loop.withIndex(0, totalCellNumber, new Loop.Each() {
		             public void run(int n) {
		            	 ApicalMeristemCenterBased3DModel cellBM = ((ApicalMeristemCenterBased3DModel)allCells.get(n).getEpisimBioMechanicalModelObject()); 
	   		...
	   		// Loop when no parallelization
	   			...
	   			for(int cellNo = 0; cellNo < totalCellNumber; cellNo++){
	   				ApicalMeristemCenterBased3DModel cellBM = ((ApicalMeristemCenterBased3DModel)allCells.get(cellNo).getEpisimBioMechanicalModelObject()); 
