...
public class FishEyeCenterBased3DModel extends AbstractCenterBased3DModel
{
    ...
    public FishEyeCenterBased3DModel(AbstractCell cell, EpisimModelConnector modelConnector)
    {
     	...
     	{
	  	    if(ModelController.getInstance().getEpisimBioMechanicalModelGlobalParameters() instanceof FishEyeCenterBased3DModelGP)
	  	    {
        ...
        if(cell != null && cell.getMotherCell() != null)
        {
	        if(cell.getMotherCell().getEpisimBioMechanicalModelObject() instanceof FishEyeCenterBased3DModel)
	        {
	      	    ...
	      	    {
	      		    // Inherit dimensions and other properties from mother cell
	      	    	// RADIUS REF //
    ...
    {
		dummyCellField.clear(); // reset dummy cells at every simulation step
		...
		/*
		 * First, calculate how many dummy cells should fit on the edge of the hemisphere.
		 * Then, use this number to calculate the angle between adjacent dummy cells.
		 * Go around the circle in increments of this angle and place the cells there.
		 */
	    // RADIUS REF //
    ...
    public InteractionResult calculateRepulsiveAdhesiveAndChemotacticForces(Bag neighbours, Double3D thisloc, boolean finalSimStep)
    {
        ...
        /*
         *  Calculate forces exerted by interactions with dummy neighbors at boundary.
         *  There are only repulsive interactions with dummy cells, no adhesion.
         */
        if(dummyCellsAdded)
        {
      	    ...
      	    // Loop through dummy cell neighbors
      	    for(int i = 0; i < dummyNeighbours.size(); i++)
      	    {
	            ...
	            double requiredDistanceToMembraneThis = calculateDistanceToCellCenter(thislocP, otherPosToroidalCorrection(thislocP, otherlocP), thisSemiAxisA, thisSemiAxisB, thisSemiAxisC);
	            double requiredDistanceToMembraneOther = calculateDistanceToCellCenter(otherlocP, otherPosToroidalCorrection(otherlocP, thislocP), otherSemiAxisA, otherSemiAxisB, otherSemiAxisC);            
	            double optDistScaled = (requiredDistanceToMembraneThis+requiredDistanceToMembraneOther)*globalParameters.getOptDistanceScalingFactor();
	            double optDist = (requiredDistanceToMembraneThis+requiredDistanceToMembraneOther);    
        ...
        // Obtain cell-cell contact area in the final iteration of the mechanical model
        // The contact to the hemispherical surface is also calculated, but is currently not used in the model.
        ...
        if(this.modelConnector instanceof episimmcc.centerbased3d.fisheye.EpisimFishEyeCenterBased3DMC && finalSimStep)
        {
      	    ...
      	    double requiredDistanceToMembraneThis = calculateDistanceToCellCenter(thislocP, otherPosToroidalCorrection(thislocP, globalParameters.getInnerEyeCenter()), thisSemiAxisA, thisSemiAxisB, thisSemiAxisC);
      	    ...
      	    // RADIUS REF //
      	    double contactAreaInnerEyeCorrect = calculateContactAreaNew(thisLocPoint, otherPosToroidalCorrection(thisLocPoint, globalParameters.getInnerEyeCenter()),
    ...
    /*
     *  Function to calculate contact area (e.g. cell-cell or cell-hemisphere).
	...
	// This function is used both for cell-cell distance calculation and cell-eye center distance
...
//		rayDirection.scale((sphradius + cellMembraneToCenterDistance));
		...
		// Get cell-center to cell-membrane distance, scale by optimal distance factor
   	    ...
   	    // Quick-and-dirty way to turn off cell adjustment:
