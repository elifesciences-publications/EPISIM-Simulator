...
public class ExtraCellularDiffusionInitializer {
	...
	public EpisimPortrayal[] getExtraCellularDiffusionPortrayals(){	
		if(ModelController.getInstance().getExtraCellularDiffusionController().getNumberOfEpisimExtraCellularDiffusionFieldConfigurations() >0){
			ExtraCellularDiffusionField[] diffusionFields = ModelController.getInstance().getExtraCellularDiffusionController().getAllExtraCellularDiffusionFields();	
			...
			for(int i = 0; i < diffusionFields.length; i++){
				currentDiffusionFieldPortrayals[i] = new ExtraCellularDiffusionPortrayal(diffusionFields[i]);
			}			
		}
		else{
	...
	private void initializeExtraCellularDiffusionFields(ExtraCellularDiffusionField[] diffusionFields){
	...
	private void rebuildExtraCellularDiffusionFields(){
		if(ModelController.getInstance().getExtraCellularDiffusionController().getNumberOfEpisimExtraCellularDiffusionFieldConfigurations() >0){
			...
			for(int i = 0; i< fieldConfigurations.length; i++){
				actField = new ExtraCellularDiffusionField(fieldConfigurations[i],widthInMikron, heightInMikron, true);
				
				//TODO Remove this initialization loops
				int delta = 8;				
				int width_half = actField.getExtraCellularField().getWidth()/2;
				int height_half = actField.getExtraCellularField().getHeight()/2;
